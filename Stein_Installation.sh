...
echo "GRANT ALL PRIVILEGES ON iotronic.* TO 'iotronic'@'localhost' IDENTIFIED BY 'IOTRONIC_DBPASS';" | $maria_db_connect
echo "GRANT ALL PRIVILEGES ON iotronic.* TO 'iotronic'@'%' IDENTIFIED BY 'IOTRONIC_DBPASS';" | $maria_db_connect
echo "CREATE DATABASE designate;" | $maria_db_connect
echo "GRANT ALL PRIVILEGES ON designate.* TO 'designate'@'localhost' IDENTIFIED BY 'DESIGNATE_DBPASS';" | $maria_db_connect
echo "GRANT ALL PRIVILEGES ON designate.* TO 'designate'@'%' IDENTIFIED BY 'DESIGNATE_DBPASS';" | $maria_db_connect
...
keystone-manage bootstrap --bootstrap-password Opst_stfth \
...
openstack user create --domain default --password uer_pswrd myuser
...
openstack --os-auth-url http://controller:5000/v3 \
  ...
  --os-project-name admin --os-username admin --os-password Opst_stfth token issue

#As the myuser user created in the previous, request an authentication token
openstack --os-auth-url http://controller:5000/v3 \
  --os-project-domain-name Default --os-user-domain-name Default \
  --os-project-name myproject --os-username myuser --os-password uer_pswrd token issue
}
...
maria_db_root_password="smartme"
